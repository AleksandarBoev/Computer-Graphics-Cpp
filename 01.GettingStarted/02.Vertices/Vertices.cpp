...
void drawDynamicTriangle(GLFWwindow *window, float vertices[], int& shaderProgram, int VAO);
...
int main()
{
	...
	std::cout << "Type in 1 for static triangle and 2 for moving triangle: ";
	short int option;
	std::cin >> option;
	...
	*/
	...
	if (option == 1)
	{
		...
		drawStaticTriangle(window, vertices, shaderProgram, VAO);
	}
	else if (option == 2) //TODO for some reason it doesn't work when its in a function
	{
